...
CREATE TABLE IF NOT EXISTS `nmu`.`AirticketTour` (
  ...
  `flight_code` VARCHAR(45) NULL COMMENT '航司大编码',
  `locators` VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `nmu`.`QuestionBoard` (
  ...
  `more_question_details` VARCHAR(255) NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `nmu`.`AirTicketTourOrder` (`transaction_id` INT, `'create_time'` INT, `salesperson_code` INT, `'name'` INT, `currency` INT, `payment_type` INT, `total_profit` INT, `received` INT, `received2` INT, `expense` INT, `coupon` INT, `flight_code` INT, `locators` INT, `ticket_type` INT, `round_trip` INT, `'passenger'` INT, `all_schedule` INT, `refunded` INT, `source_name` INT, `note` INT, `clear_status` INT, `lock_status` INT);
...
CREATE  OR REPLACE VIEW AirTicketTourOrder AS
    SELECT 
        ...
        a.ticket_type,
        a.round_trip,
        CONCAT(a.adult_number,
                ' / ',
                a.child_number,
                ' / ',
                a.infant_number) AS 'passenger',
        asi.all_schedule,
        a.refunded,
        sn.source_name,
        t.note,
    ...
    FROM
        AirticketTour a
            LEFT JOIN
        AirScheduleIntegrated asi ON a.airticket_tour_id = asi.airticket_tour_id
            INNER JOIN
        Transactions t ON a.airticket_tour_id = t.airticket_tour_id
            LEFT JOIN
        Salesperson s ON t.salesperson_id = s.salesperson_id
            LEFT JOIN
        CustomerSource sn ON t.source_id = sn.source_id
            LEFT JOIN
