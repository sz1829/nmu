...
SELECT received FROM GroupTour WHERE group_tour_id = OLD.group_tour_id INTO @received;
SET @old_received = OLD.received;
IF OLD.received_currency = @received_currency THEN
    UPDATE GroupTour SET received = received - OLD.received WHERE group_tour_id = OLD.group_tour_id;
ELSE 
