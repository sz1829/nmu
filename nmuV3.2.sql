...
-- Wed 19 Sep 2018 03:46:28 PM EDT
...
CREATE TABLE IF NOT EXISTS `nmu`.`Wholesaler` (
  ...
  UNIQUE INDEX `wholesaler_code_UNIQUE` (`wholesaler_code` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `nmu`.`Salesperson` (
  ...
  INDEX `fk_dep_id_idx` (`department_id` ASC) VISIBLE,
  INDEX `fk_s_ta_id_idx` (`ta_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`CouponCode` (
  ...
  INDEX `cc_fk_salesperson_id_idx` (`salesperson_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`GroupTour` (
  ...
  UNIQUE INDEX `tour_id_UNIQUE` (`group_tour_id` ASC) VISIBLE,
  INDEX `fk_salesperson_id_idx` (`salesperson_id` ASC) VISIBLE,
  INDEX `gt_cc_id_idx` (`cc_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`IndividualTour` (
  ...
  `commission_fee` DECIMAL(11,2) NULL,
  ...
  INDEX `fk_wholesaler_id_idx` (`wholesaler_id` ASC) VISIBLE,
  INDEX `fk_salesperson_id_idx` (`salesperson_id` ASC) VISIBLE,
  INDEX `fk_customer_id_indiv_idx` (`customer_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`AirticketTour` (
  ...
  INDEX `at_ta_id_idx` (`ta_id` ASC) VISIBLE,
  INDEX `at_cc_id_idx` (`cc_id` ASC) VISIBLE,
  INDEX `at_wholesaler_id_idx` (`wholesaler_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`CustomerSource` (
  ...
  UNIQUE INDEX `sourse_name_UNIQUE` (`source_name` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `nmu`.`UserAccount` (
  ...
  PRIMARY KEY (`user_id`),
  INDEX `fk_group_id_idx` (`user_group_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`NoticeBoard` (
  ...
  INDEX `fk_user_id_nb_idx` (`edited_by` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`McoInfo` (
  ...
  PRIMARY KEY (`mco_id`),
  INDEX `fk_McoInfo_notice_id_idx` (`notice_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`Transactions` (
  ...
  INDEX `fk_salesperson_id_idx` (`salesperson_id` ASC) VISIBLE,
  INDEX `fk_indiv_id_idx` (`indiv_tour_id` ASC) VISIBLE,
  INDEX `fk_airticket_id_idx` (`airticket_tour_id` ASC) VISIBLE,
  INDEX `fk_source_id_idx` (`source_id` ASC) VISIBLE,
  UNIQUE INDEX `group_tour_id_UNIQUE` (`group_tour_id` ASC) VISIBLE,
  UNIQUE INDEX `indiv_tour_id_UNIQUE` (`indiv_tour_id` ASC) VISIBLE,
  UNIQUE INDEX `airticket_tour_id_UNIQUE` (`airticket_tour_id` ASC) VISIBLE,
  INDEX `t_mco_id_idx` (`mco_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`QuestionBoard` (
  ...
  INDEX `fk_ask_salesperson_id_idx` (`ask_salesperson_id` ASC) VISIBLE,
  INDEX `fk_qb_ta_id_idx` (`ta_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`AirSchedule` (
  ...
  PRIMARY KEY (`as_id`),
  INDEX `fk_as_airticket_tour_id_idx` (`airticket_tour_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`AirScheduleIntegrated` (
  ...
  INDEX `fk_asi_at_id_idx` (`airticket_tour_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`ThingsToDo` (
  ...
  PRIMARY KEY (`tto_id`),
  INDEX `fk_user_id_tto_idx` (`user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`FrequentWords` (
  ...
  INDEX `fk_fw_user_id_idx` (`user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`GroupTourGuideDetails` (
  ...
  PRIMARY KEY (`gd_id`),
  INDEX `fk_group_tour_id_gtg_idx` (`group_tour_id` ASC) VISIBLE,
  INDEX `fk_tour_guide_gtg_idx` (`guide_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`GroupTourReceived` (
  ...
  `received_currency` ENUM('USD', 'RMB') NULL DEFAULT 'USD',
  `payment_type` ENUM('creditcard', 'mco', 'alipay', 'wechat', 'cash', 'check', 'other', 'multiple', 'remit') NULL,
  `commission_fee` DECIMAL(11,2) NULL,
  PRIMARY KEY (`gtr_id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `nmu`.`UpdateLog` (
  ...
  PRIMARY KEY (`log_id`),
  INDEX `ul_fk_transaction_id_idx` (`transaction_id` ASC) VISIBLE,
  INDEX `ul_fk_user_id_idx` (`revised_by` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`NoticeTarget` (
  ...
  `target_id` INT NULL,
  PRIMARY KEY (`nt_id`),
  INDEX `fk_nt_notice_id_idx` (`notice_id` ASC) VISIBLE,
  INDEX `fk_nt_target_id_idx` (`target_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nmu`.`WholesalerCollection` (
  ...
  `group_tour_id` INT NULL,
  PRIMARY KEY (`wc_id`),
  INDEX `wc_fk_wholesaler_id_idx` (`wholesaler_id` ASC) VISIBLE,
  INDEX `wk_fk_group_tour_id_idx` (`group_tour_id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `nmu`.`ReceivedConfirmed`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `nmu`.`ReceivedConfirmed` (
  `rc_id` INT NOT NULL AUTO_INCREMENT,
  `transaction_id` INT NULL,
  `sent_by` INT NULL,
  `process` ENUM('pending', 'confirmed') NULL,
...
CREATE TABLE IF NOT EXISTS `nmu`.`LogLastEditor` (
  ...
  `user_id` INT NULL,
  PRIMARY KEY (`lle_id`),
  INDEX `lle_fk_transaction_id_idx` (`transaction_id` ASC) VISIBLE,
  INDEX `lle_fk_user_id_idx` (`user_id` ASC) VISIBLE,
