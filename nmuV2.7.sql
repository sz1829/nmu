...
CREATE TABLE IF NOT EXISTS `nmu`.`McoInfo` (
  ...
  INDEX `fk_McoInfo_transaction_id_idx` (`transaction_id` ASC),
  CONSTRAINT `fk_McoInfo_notice_id`
    FOREIGN KEY (`notice_id`)
    REFERENCES `nmu`.`NoticeBoard` (`notice_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_McoInfo_transaction_id`
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `nmu`.`GroupTourOrder` (`transaction_id` INT, `'create_time'` INT, `group_code` INT, `salesperson_code` INT, `'total_number'` INT, `currency` INT, `payment_type` INT, `'profit'` INT, `price` INT, `'cost'` INT, `'coupon'` INT, `flight_number` INT, `bus_company` INT, `'schedule'` INT, `'guide_name'` INT, `'guide_phone'` INT, `'guide_write_off'` INT, `'agency_name'` INT, `source_name` INT, `clear_status` INT, `lock_status` INT, `note` INT);
